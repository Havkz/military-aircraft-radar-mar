...
- Authorized Flightradar24 and ADS-B Exchange visible-map sources on Android, loaded in embedded background WebViews only while MAR's map is visible
...
2. MAR requests nearby-aircraft data from ADSB.lol and Airplanes.live over HTTPS. If configured, it also requests ADS-B Exchange through its official authenticated API. While Android's map is visible, authorized embedded Flightradar24 and ADS-B Exchange pages follow the same visible viewport and supply their ICAO-addressed contacts.
...
MAR contains no analytics, advertising, crash-reporting, or user-account SDKs. It has no developer-operated server. While monitoring is active, location-related request data is sent directly from the device to ADSB.lol and Airplanes.live, and to the ADS-B Exchange API only when an API key is configured. While Android's map is visible, embedded Flightradar24 and ADS-B Exchange pages load the same map center and zoom and contribute their visible aircraft to the on-device merge. Viewing the map also requests OpenStreetMap tiles for the visible area. Selecting an external tracker transfers aircraft or map-position information to that service.
...
MAR combines information from [ADSB.lol](https://www.adsb.lol/), [Airplanes.live](https://airplanes.live/), and authorized embedded [Flightradar24](https://www.flightradar24.com/) and [ADS-B Exchange](https://globe.adsbexchange.com/) visible-map sessions on Android, and can optionally use the official authenticated ADS-B Exchange API. Map tiles are provided by OpenStreetMap with visible attribution. The app includes provider notices in Settings under **Legal & Data Sources**.

The Flightradar24 and ADS-B Exchange visible-map integrations are covered by project-specific permissions granted to the maintainer; those permissions do not automatically extend to forks or redistributors. MAR does not scrape Airplanes.live or other provider websites. Provider names and trademarks belong to their respective owners; no affiliation or endorsement is claimed.
