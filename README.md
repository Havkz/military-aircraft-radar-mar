...
No account, subscription, advertising SDK, analytics SDK, or aircraft-data API key is required.
...
4. Open **Settings** to choose units, language, theme, vibration, and the tracker used when an aircraft notification is selected.
...
2. MAR requests nearby-aircraft data from ADSB.lol and Airplanes.live over HTTPS. While Android's map is visible, authorized embedded Flightradar24 and ADS-B Exchange pages follow the same visible viewport and supply their ICAO-addressed contacts.
...
MAR contains no analytics, advertising, crash-reporting, or user-account SDKs. It has no developer-operated server. While monitoring is active, location-related request data is sent directly from the device to ADSB.lol and Airplanes.live. While Android's map is visible, embedded Flightradar24 and ADS-B Exchange pages load the same map center and zoom and contribute their visible aircraft to the on-device merge. Viewing the map also requests OpenStreetMap tiles for the visible area. Selecting an external tracker transfers aircraft or map-position information to that service.
...
MAR combines information from [ADSB.lol](https://www.adsb.lol/), [Airplanes.live](https://airplanes.live/), and authorized embedded [Flightradar24](https://www.flightradar24.com/) and [ADS-B Exchange](https://globe.adsbexchange.com/) visible-map sessions on Android. Map tiles are provided by OpenStreetMap with visible attribution. The app includes provider notices in Settings under **Legal & Data Sources**.
