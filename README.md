...
- Authorized Flightradar24 visible-map source on Android, loaded in an embedded background WebView only while MAR's map is visible
...
2. MAR requests nearby-aircraft data from ADSB.lol and Airplanes.live over HTTPS. If configured, it also requests ADS-B Exchange through its official authenticated API. While Android's map is visible, an authorized embedded Flightradar24 page follows the same visible viewport and supplies its ICAO-addressed contacts.
...
MAR contains no analytics, advertising, crash-reporting, or user-account SDKs. It has no developer-operated server. While monitoring is active, location-related request data is sent directly from the device to ADSB.lol and Airplanes.live, and to ADS-B Exchange only when an API key is configured. While Android's map is visible, an embedded Flightradar24 page loads the same map center and zoom and contributes its visible aircraft to the on-device merge. Viewing the map also requests OpenStreetMap tiles for the visible area. Selecting an external tracker transfers aircraft or map-position information to that service.
...
MAR combines information from [ADSB.lol](https://www.adsb.lol/), [Airplanes.live](https://airplanes.live/), and an authorized embedded [Flightradar24](https://www.flightradar24.com/) visible-map session on Android, and can optionally use the official authenticated ADS-B Exchange API. Map tiles are provided by OpenStreetMap with visible attribution. The app includes provider notices in Settings under **Legal & Data Sources**.

The Flightradar24 integration is covered by project-specific permission granted to the maintainer; that permission does not automatically extend to forks or redistributors. MAR does not scrape ADS-B Exchange, Airplanes.live, or other provider websites. Provider names and trademarks belong to their respective owners; no affiliation or endorsement is claimed.
