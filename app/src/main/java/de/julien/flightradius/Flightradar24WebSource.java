...
final class Flightradar24WebSource {
    ...
    private static final String EXPORT_HOOK = "(function(){"
            ...
            + ".replace(/[^A-Z0-9]/g,'')}"
            + "function contactKey(f){const c=norm(f.callsign),r=norm(f.registration);"
            + "return c?'c:'+c:r?'r:'+r:'f:'+String(f.flightId||f.icao).toLowerCase()}"
            + "function quality(f){return(!String(f.icao).startsWith('~')?8:0)"
            + "+(norm(f.registration)?6:0)+(norm(f.type)?6:0)"
            + "+(norm(f.callsign)?2:0)+(finite(Number(f.timestamp))?1:0)}"
            + "function prefer(next,current){const n=quality(next),c=quality(current);"
            + "if(n!==c)return n>c;return Number(next.timestamp||next.timestampMs||0)"
            + ">=Number(current.timestamp||current.timestampMs||0)}"
            ...
            + "{const key=contactKey(flight),current=pending.get(key);"
            + "if(!current||prefer(flight,current))pending.set(key,flight)}if(!timer)"
