...
If worldwide squawk notifications are enabled, Android sends the selected four-digit squawk code, or the `7500`, `7600`, and `7700` emergency-code preset, to the global squawk endpoints of ADSB.lol and Airplanes.live. ADS-B Exchange is included only when the user has configured an official API key. These worldwide requests do not include the device location. On the Airplanes.live Free plan, map and squawk requests are spaced to share its published daily allowance; an explicitly authorized Business setting uses the selected one- or five-minute interval. Alert deduplication state remains in memory on the device.
