...
App preferences, recent aircraft state, map display options, and map bookmarks are stored locally in platform-managed application or WebView storage. Search terms and aircraft filters remain on the device. If the optional “include filters in map URL” setting is enabled, the active filters are encoded only in the local WebView URL fragment; URL fragments are not sent in HTTP requests. These values can be removed by clearing the Android app's data or uninstalling the app. Updates from a version that supported an ADS-B Exchange API key delete the obsolete key from Android's private no-backup directory when the app or radar service next starts.
